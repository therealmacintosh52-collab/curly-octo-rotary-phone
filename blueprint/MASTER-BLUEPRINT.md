...
The reference build is `sites/cali-tints/` (Cali Tints, Sacramento), which superseded `sites/automotive-solutions-v2/` and the original `sites/phils-auto-v2/`. Every new site is
...
```bash
cp -r sites/cali-tints sites/<new-slug>
cd sites/<new-slug>
rm -rf node_modules dist dist-preview qa public/assets/video/* public/assets/img/hero-poster.* public/*[0-9a-f].txt .indexnow-key
