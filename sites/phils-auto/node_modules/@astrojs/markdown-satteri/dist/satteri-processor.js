  ...
  return {
    ...
    element: {
      ...
      visit(node, ctx) {
        ...
        const languageClass = Array.isArray(codeChild.properties?.className) ? codeChild.properties.className.find(
          (className) => typeof className === "string" && className.startsWith("language-")
        ) : void 0;
        const lang = codeChild.data?.lang ?? languageClass?.slice("language-".length) ?? "plaintext";
