...
function instrumentFetchRequest(handlerData, shouldCreateSpan, shouldAttachHeaders, spans, spanOriginOrOptions) {
  ...
  const span = shouldCreateSpanResult && shouldEmitSpan ? trace.startInactiveSpan(getSpanStartOptions(url, method, spanOrigin, client)) : new sentryNonRecordingSpan.SentryNonRecordingSpan();
...
function getSpanStartOptions(url$1, method, spanOrigin, client) {
  if (url$1.startsWith("data:")) {
    const sanitizedUrl2 = url.stripDataUrlContent(url$1);
    return {
      name: `${method} ${sanitizedUrl2}`,
      attributes: getFetchSpanAttributes(url$1, void 0, method, spanOrigin, client)
    };
  }
  ...
  return {
    ...
    attributes: getFetchSpanAttributes(url$1, parsedUrl, method, spanOrigin, client)
  };
}
function getFetchSpanAttributes(url$1, parsedUrl, method, spanOrigin, client) {
  const attributes$1 = {
    url: filterCollectedUrl.filterCollectedUrl(url.stripDataUrlContent(url$1), client),
    type: "fetch",
    "http.method": method,
  ...
  if (parsedUrl) {
    if (!url.isURLObjectRelative(parsedUrl)) {
      attributes$1[attributes.HTTP_URL] = filterCollectedUrl.filterCollectedUrl(url.stripDataUrlContent(parsedUrl.href), client);
      attributes$1[attributes.URL_FULL] = filterCollectedUrl.filterCollectedUrl(url.stripDataUrlContent(parsedUrl.href), client);
      attributes$1["server.address"] = parsedUrl.host;
    }
    if (parsedUrl.search) {
      attributes$1["http.query"] = filterCollectedUrl.filterCollectedUrlQuery(parsedUrl.search, client);
    }
    if (parsedUrl.hash) {
