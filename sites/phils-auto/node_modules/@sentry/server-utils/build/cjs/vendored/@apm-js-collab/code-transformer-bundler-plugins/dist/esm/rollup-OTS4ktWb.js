...
function codeTransformerRollup(options) {
	const { transform: transformCode, getCodeToInject } = coreDC9TN3Ev.r(options);
	let sourcemapsEnabled = false;
	const outputOptions = (inputOptions) => {
		sourcemapsEnabled = !!inputOptions.sourcemap;
	};
	const renderChunk = (code, chunk, _, meta) => {
		if (!coreDC9TN3Ev.i(chunk.fileName)) return null;
		if (!chunk.isEntry) return null;
		if (coreDC9TN3Ev.a(code, chunk.facadeModuleId)) return null;
		...
		const match = code.match(coreDC9TN3Ev.t)?.[0];
...
exports.t = codeTransformerRollup;
//# sourceMappingURL=rollup-OTS4ktWb.js.map
